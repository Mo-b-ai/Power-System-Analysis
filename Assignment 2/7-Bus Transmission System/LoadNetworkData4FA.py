...
def LoadNetworkData4FA(filename):
    ...
    for line in line_data:
        ...
        # edit: For the fault analysis the assumption is made that the contribution from shunt components are negligible and are therefore not included.
    ...
    for line in tran_data:
        ...
        #edit: the postive sequence network for the transformer follows the [[Y/n^2],[-Ye/n],[-Ye/n],[Ye]]. The phase shift is neglected. 
        #positive sequence
        Zeq = 1j*X; Yeq = 1/Zeq
        ...
        #edit: the negative sequence is identical to the postive sequence with the negative sequence reactance.
        ...
        if fr_co == 2 and to_co == 2:
            ...
            Yps_mat[1,0] = -Y0/n
            Yps_mat[1,1] = Y0
        elif fr_co == 2 and to_co == 3: #edit: If the transformer is connected in Yg-D, the only connection is from-con to ground = [0,0]= Y0/n**2. All others = 0
            Yps_mat[0,0] = Y0/n**2
        elif fr_co == 3 and to_co == 2: #edit: If the transformer is connected in D-Yg, the only connection is to-con to ground = [0,0]= Y0. All others = 0
            Yps_mat[1,1] = Y0   
        ind_ = np.array([ind_fr,ind_to])
    ...
    for line in gen_data:
        ...
        Y = 1/Z        
        #Update the bus admittance matrix
        Ybus[ind_bus,ind_bus]+= Y #edit: positive sequence is only the Y1 admittance
        #negative sequence
        Z2 = 1j*X2*mva_base/MVA_size;
        Y2 = 1/Z2
        Ybus2[ind_bus,ind_bus]+= Y2 #edit: negative sequence is only the Y2 admittance
        #zero sequence
        Z0 = 1j*X0*mva_base/MVA_size 
        if ground:
            Z0 += 1j*( 3*Xn) #edit: zero sequence is the 1/(Z0+3Zn) admittance
        Y0 = 1/Z0
